...
- Работаешь только в своей папке `mini-wallet/mini-wallet/Features/<твоя>`.
  Общий контракт не трогаешь.
- Новые .swift-файлы просто клади в свою папку — проект подхватит сам,
  вручную добавлять их в `project.pbxproj` не надо. Настройки проекта и таргета
  менять можно, но это зона владельца корневых файлов: сначала в чат.
